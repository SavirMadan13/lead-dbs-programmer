...
function PlyViewer({ quantities, amplitude, side }) {
  ...
  const addMeshToScene = (name, geometry, material) => {
    ...
    Object.keys(contactDirections).forEach((contactId) => {
      ...
      if (contactQuantity > 0) {
        ...
        // if (!VTASpheresRef[contactId]) {
          ...
          // Create a new sphere
          const geometry = new THREE.SphereGeometry(Math.sqrt((contactAmplitude - 0.1) / 0.22), 32, 32);
